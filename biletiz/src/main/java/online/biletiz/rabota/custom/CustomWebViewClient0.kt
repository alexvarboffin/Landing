...
import com.walhalla.webview.R
import com.walhalla.webview.ReceivedError
import com.walhalla.webview.WVTools.extractDomain
import com.walhalla.webview.WVTools.loadBlockedDomains
import com.walhalla.webview.WebViewAppConfig
import com.walhalla.webview.utility.ActivityUtils
import com.walhalla.webview.utility.ActivityUtils.startCallActivity
import com.walhalla.webview.utility.ActivityUtils.startMapSearchActivity
import com.walhalla.webview.utility.ActivityUtils.startMapYandex
import com.walhalla.webview.utility.ActivityUtils.startSmsActivity
import com.walhalla.webview.utility.ActivityUtils.starttg
import com.walhalla.webview.utility.ActivityUtils.startyandexnavi
import com.walhalla.webview.utility.DownloadUtility.downloadFile
import com.walhalla.webview.utility.DownloadUtility.getFileName
import com.walhalla.webview.utility.DownloadUtility.isLinkInternal
import java.io.ByteArrayInputStream
import java.util.Locale


open class CustomWebViewClient0(
    ...
    * /        if (webResourceResponse == null)
    ...
    private fun handleUrl(view: WebView, url: String): Boolean {
        ...
        if (var0) {
            ...
            downloadFile(context, url, getFileName(url))
            return true
...
//            }
            ...
            startyandexnavi(context, url)
            return true
        } else if (url.startsWith("intent://")) {
            if (url.startsWith("intent://maps.yandex")) {
                startMapYandex(context, url.replace("intent://", "https://"))
                return true
            }
...
//                    webView.invalidate();
    ...
    init {
        ...
        this.linksOpenedInExternalBrowser =
            context.resources.getStringArray(R.array.links_opened_in_external_browser)
        this.blockedDomains0 = loadBlockedDomains(context, R.raw.blockedhost)
    }
...
//                        .show();
//        }
        val failureUrl = request.getUrl().toString()

        val mainUrl: String = (if (view.getUrl() == null) "" else view.getUrl())!!
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            ...
            if (mainUrl == failureUrl) {
                Log.d(
                    TAG,
                    "[onReceived--HttpError >= 21 ] " + error + " " + request.getUrl() + " " + view.getUrl()
                )
                //m404();
            }
        } else {
    ...
    private fun nonNull(o: Any?): Boolean {
        Log.d(TAG, "Nonnull" + (if (o != null) o.javaClass.getCanonicalName() else null))
        return o != null
    }
...
//        }
        if (statusCode == 403) {
            if (view.getUrl() != null && view.getUrl() == cUrl) {
                //m404();
            }
        }
    }
